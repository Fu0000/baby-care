...
export default function Settings() {
  ...
  async function handleExport() {
    const [sessions, contractionSessions, contractions, hospitalBagItems, feedingRecords] = await Promise.all([
      db.sessions.toArray(),
    ...
    const data = JSON.stringify({ sessions, contractionSessions, contractions, hospitalBagItems, feedingRecords }, null, 2)
  ...
  async function handleImport() {
    ...
    input.onchange = async () => {
      ...
      if (Array.isArray(data)) {
        ...
        if (data.feedingRecords) { await db.feedingRecords.bulkPut(data.feedingRecords); count += data.feedingRecords.length }
      }
